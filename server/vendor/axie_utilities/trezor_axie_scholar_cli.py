...
def generate_transfers_file(csv_file_path, transfer_file_path=None):
    ...
    for d in transfers_dict:
        transfers_list.append(
            {"AccountAddress": d, "Transfers": transfers_dict[d]["Transfers"]}
        )
...
def check_file(file):
    if not os.path.isfile(file):
        logging.critical(
            "Please provide a correct path to the file. " f"Path provided: {file}"
        )
        return False
    return True
...
def run_cli():
    ...
    if args["payout"]:
        ...
        if check_file(payments_file_path) and check_file(config_file_path):
            ...
            if args["--yes"]:
                logging.info(
                    "Automatic acceptance active, it won't ask before each execution"
                )
            apm = TrezorAxiePaymentsManager(
                payments_file_path, config_file_path, auto=args["--yes"]
            )
            apm.verify_inputs()
            apm.prepare_payout()
        else:
        ...
        if (
            config_file_path
            and check_file(config_file_path)
            and check_file(payments_file_path)
            or not config_file_path
            and check_file(payments_file_path)
        ):
            logging.info(
                "You will be asked to introduce passphrases and number of accounts per passphrase until you "
        ...
        if (
            transfers_file_path
            and check_file(transfers_file_path)
            and check_file(csv_file_path)
            or not transfers_file_path
            and check_file(csv_file_path)
        ):
        ...
        if check_file(transfers_file_path) and check_file(config_file_path):
            atm = TrezorAxieTransferManager(
                transfers_file_path, config_file_path, secure=secure
            )
            atm.verify_inputs()
        ...
        if check_file(config_file_path):
            ...
            for acc in accs_list:
                ...
                if axies_to_morph:
                    axm = TrezorAxieMorphingManager(
                        axies_to_morph, acc, config_file_path
                    )
                    axm.verify_inputs()
                    axm.execute()
                else:
        ...
        if check_file(breedings_file_path) and check_file(config_file_path):
            ...
            while payment_account == "":
                msg = input(
                    "Provide ronin account that will pay the fee for breeding: "
                )
                if len(msg) == 46 and msg.startswith("ronin:"):
                    # Make sure is a valid Hex
                    try:
                    ...
                    logging.info(f"Ronin provided ({msg}) looks wrong, try again.")
            abm = TrezorAxieBreedManager(
                breedings_file_path, config_file_path, payment_account
            )
            abm.verify_inputs()
            abm.execute()
        else:
        ...
        if (
            breedings_file_path
            and check_file(breedings_file_path)
            and check_file(csv_file_path)
            or not breedings_file_path
            and check_file(csv_file_path)
        ):
