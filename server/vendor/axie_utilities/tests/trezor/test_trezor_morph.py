...
def test_morph_manager_verify_input_fail(tmpdir, caplog):
    ...
    assert (
        f"Account '{scholar_acc}' is not present in trezor config, please re-run trezor setup."
        in caplog.text
    )
...
def test_morph_manager_execute(
    mock_morph_init, mock_morph_execute, mock_client, tmpdir
):
    scholar_acc = "ronin:<account_s1_address>" + "".join(
        [str(x) for x in range(10)] * 2
    )
    ...
    mock_morph_init.assert_has_calls(
        calls=[
            call(
                axie=1,
                account=scholar_acc,
...
def test_morph_init(mock_parse):
    m = TrezorMorph(
        axie=1, account="ronin:abcd1", client="client", bip_path="m/44'/60'/0'/0/0"
    )
...
@patch("trezor.trezor_morphing.ethereum.sign_message", return_value=MockedSignedMsg())
@patch("trezor.trezor_morphing.TrezorMorph.get_jwt", return_value="token")
def test_morph_execute_bad_json_response(
    mock_get_jwt, mock_sign_msg, mock_parse, caplog
):
    with requests_mock.Mocker() as req_mocker:
        req_mocker.post(
            "https://graphql-gateway.axieinfinity.com/graphql", json={"foo": "bar"}
        )
        m = TrezorMorph(
            axie=1, account="ronin:abcd1", client="client", bip_path="m/44'/60'/0'/0/0"
        )
