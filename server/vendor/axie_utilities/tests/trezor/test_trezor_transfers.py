...
def test_transfer_manager_init(mocked_load_json):
    ...
    mocked_load_json.assert_has_calls(calls=[call(transfers_file), call(config_file)])
...
def test_transfer_manager_prepare_transfers(
    mocked_execute_transfers, mocked_load_json, mock_axies, mock_client, mock_parse
):
    transfers_file = "sample_transfers_file.json"
    config_file = "sample_config_file.json"
    atm = TrezorAxieTransferManager(transfers_file, config_file)
