...
def test_payments_manager_init(mocked_load_json):
    ...
    mocked_load_json.assert_has_calls(calls=[call(payments_file), call(config_file)])
...
def test_payments_manager_verify_input_correct_dono(tmpdir, caplog):
    ...
    mocked_sys.assert_called_once()
    assert (
        "Account 'Scholar 1' is not present in trezor config file, please re-run setup."
        in caplog.text
    )
...
@patch(
    "trezor.TrezorAxiePaymentsManager.check_acc_has_enough_balance", return_value=True
)
def test_payments_manager_prepare_payout_check_correct_payments(
    mocked_enough_balance,
    mocked_payout,
...
def test_payments_manager_prepare_payout_check_correct_payments_no_balance(
    mocked_enough_balance, mocked_payout, mocked_check_balance, mocked_client, tmpdir
):
    p_file = tmpdir.join("p.json")
    scholar_acc = "ronin:12345678900987654321" + "".join(
        [str(x) for x in range(10)] * 2
