...
def test_breed_manager_init(mock_load):
    ...
    mock_load.assert_has_calls(calls=[call(c_file), call(b_file)])
...
def test_breed_manager_verify_inputs_missing_account_in_secrets(tmpdir, caplog):
    ...
    mocked_sys.assert_called_once()
    assert (
        f"Account '{acc}' is not present in trezor config, please re-run setup."
        in caplog.text
    )
...
def test_breed_manager_verify_inputs_missing_payments_account_in_secrets(
    ...
    mocked_sys.assert_called_once()
    assert (
        "Payment account 'ronin:abc1' is not present in trezor config, please re-run setup."
        in caplog.text
    )
...
def test_breed_manager_execute_not_enough_slp(
    mock_check_balance, _, __, ___, ____, mocked_client, tmpdir, caplog
):
    acc = "ronin:<accountfoo_address>" + "".join([str(x) for x in range(10)] * 4)
    c_file = tmpdir.join("c.json")
    config_data = {acc: {"passphrase": "", "bip_path": "m/44'/60'/0'/0/0"}}
