...
@patch(
    "trezor.TrezorQRCodeManager.load_trezor_config_and_acc_name",
...
def test_qrcode_manager_execute(
    mocked_qrcode_init, mocked_qrcode_generate_qr, mocked_client, tmpdir
):
    scholar_acc = "ronin:<account_s1_address>" + "".join(
        [str(x) for x in range(10)] * 4
    )
