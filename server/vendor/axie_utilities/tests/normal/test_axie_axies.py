...
def test_get_axies(mocked_number_of_axies, mocked_checksum, mocked_contract, _):
    ...
    mocked_checksum.assert_has_calls(calls=[call(AXIE_CONTRACT), call(a.acc)])
    assert len(axies) == 5
...
def test_find_axies_to_morph_none_morph_date_shape(
    mocked_number_of_axies, mocked_checksum, mocked_contract, mocked_get_data, _
):
    with patch.object(builtins, "open", mock_open(read_data='{"foo": "bar"}')):
        a = Axies("ronin:abc1")
...
@patch("web3.eth.Eth.contract.functions.tokenOfOwnerByIndex")
@patch(
    "axie.Axies.get_morph_date_and_body",
    return_value=(datetime(2021, 1, 14, 1, 0, 0), "Normal"),
)
@patch("web3.eth.Eth.contract")
@patch("web3.Web3.toChecksumAddress", return_value="checksum")
@patch("axie.Axies.number_of_axies", return_value=1)
def test_find_axies_to_morph_already_adult(
    mocked_number_of_axies, mocked_checksum, mocked_contract, mocked_get_data, _
):
    with patch.object(builtins, "open", mock_open(read_data='{"foo": "bar"}')):
        a = Axies("ronin:abc1")
...
@patch("web3.Web3.toChecksumAddress", return_value="checksum")
@patch("axie.Axies.number_of_axies", return_value=1)
def test_find_axies_to_morph_not_hatch_time(
    mocked_number_of_axies, mocked_checksum, mocked_contract, mocked_get_data, _
):
    with patch.object(builtins, "open", mock_open(read_data='{"foo": "bar"}')):
        a = Axies("ronin:abc1")
...
@patch(
    "axie.Axies.get_morph_date_and_body",
    return_value=(datetime(2021, 1, 14, 0, 0, 0), None),
)
@patch("web3.eth.Eth.contract")
@patch("web3.Web3.toChecksumAddress", return_value="checksum")
@patch("axie.Axies.number_of_axies", return_value=1)
@freeze_time("2021-01-14 01:10:05")
def test_find_axies_to_morph(
    mocked_number_of_axies, mocked_checksum, mocked_contract, mocked_get_data, _
):
    with patch.object(builtins, "open", mock_open(read_data='{"foo": "bar"}')):
        a = Axies("ronin:abc1")
...
def test_get_morph_date_body_malformed_json(mocked_json):
    with requests_mock.Mocker() as req_mocker:
        req_mocker.post(
            "https://graphql-gateway.axieinfinity.com/graphql", json=mocked_json
        )
        a = Axies("ronin:abc1")
        resp = a.get_morph_date_and_body(123)
    assert resp == (None, None)
