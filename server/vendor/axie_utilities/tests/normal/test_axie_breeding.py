...
def test_breed_manager_init(mock_load):
    ...
    mock_load.assert_has_calls(calls=[call(s_file), call(b_file)])
...
def test_breed_manager_verify_inputs_fail_validation(tmpdir, caplog):
    ...
    assert (
        f"Validation of breeding file failed. Error given: '{acc}' does not match '^ronin:'"
        in caplog.text
    )
...
def test_breed_manager_verify_inputs_missing_account_in_secrets(tmpdir, caplog):
    ...
    mocked_sys.assert_called_once()
    assert (
        f"Account '{acc}' is not present in secret file, please add it." in caplog.text
    )


def test_breed_manager_verify_inputs_missing_payments_account_in_secrets(
    tmpdir, caplog
):
    acc = "0x<accountfoo_address>" + "".join([str(x) for x in range(10)] * 4)
    private_acc = "0x<accountfoo_private_address>012345" + "".join(
    ...
    mocked_sys.assert_called_once()
    assert (
        "Payment account 'ronin:abc1' is not present in secret file, please add it."
        in caplog.text
    )
...
def test_breed_manager_calculate_fee_cost(tmpdir):
    ...
    assert abm.calculate_fee_cost() == (15 * 30) + (15 * 25) + (20 * 20) + (
        (75 - 50) * 15
    )
...
def test_breed_manager_execute(
    mock_payments_init,
    ...
    b_file = tmpdir.join("b.json")
    data = [
        {"Sire": 1234, "Matron": 5678, "AccountAddress": acc},
        {"Sire": 123, "Matron": 456, "AccountAddress": acc},
    ]
    b_file.write(json.dumps(data))
    abm = AxieBreedManager(b_file, s_file, acc)
    abm.execute()
    mock_check_balance.assert_called_once()
    mock_breed_init.assert_has_calls(
        calls=[
            call(
                sire_axie=1234, matron_axie=5678, address=acc, private_key=private_acc
            ),
...
def test_breed_manager_execute_not_enough_slp(
    mock_check_balance, _, __, ___, ____, tmpdir, caplog
):
    acc = "ronin:<accountfoo_address>" + "".join([str(x) for x in range(10)] * 4)
    private_acc = "0x<accountfoo_private_address>012345" + "".join(
        [str(x) for x in range(10)] * 3
