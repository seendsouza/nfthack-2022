...
def test_claims_manager_verify_only_accounts_in_payments_get_claimed(tmpdir):
    ...
    p_file.write(json.dumps(data))
    s_file = tmpdir.join("s.json")
    s_data = {scholar_acc: scholar_private_acc, "ronin:acc": "ronin:secret"}
...
def test_claim_init(mocked_provider, mocked_checksum, mocked_contract):
    with patch.object(builtins, "open", mock_open(read_data='{"foo": "bar"}')):
...
def test_has_unclaimed_slp(
    mocked_provider, mocked_checksum, mocked_contract, mocked_check
):
    last_claimed_date = datetime.now() - timedelta(days=15)
    with requests_mock.Mocker() as req_mocker:
        req_mocker.get(
            "https://game-api.skymavis.com/game-api/clients/0xfoo/items/1",
            json={
                "total": 12,
...
@patch("web3.Web3.toChecksumAddress", return_value="checksum")
@patch("web3.Web3.HTTPProvider", return_value="provider")
def test_has_unclaimed_slp_failed_req(
...
def test_create_random_msg():
    with requests_mock.Mocker() as req_mocker:
        ...
        resp = Claim(
            account="ronin:foo", private_key="0xbar", acc_name="test_acc"
        ).create_random_msg()
...
def test_create_random_msg_fail_req():
    with requests_mock.Mocker() as req_mocker:
        req_mocker.post(
            "https://graphql-gateway.axieinfinity.com/graphql", status_code=500
        )
        c = Claim(account="ronin:foo", private_key="0xbar", acc_name="test_acc")
...
def test_create_random_msg_fail_no_data_req():
    with requests_mock.Mocker() as req_mocker:
        req_mocker.post("https://graphql-gateway.axieinfinity.com/graphql", json={})
        c = Claim(account="ronin:foo", private_key="0xbar", acc_name="test_acc")
        random_msg = c.create_random_msg()
        assert random_msg is None
...
@patch("web3.eth.Eth.contract")
@patch(
    "web3.eth.Eth.account.sign_message", return_value={"signature": HexBytes(b"123")}
)
@patch("axie.claims.Claim.create_random_msg", return_value="random_msg")
@patch("web3.Web3.toChecksumAddress", return_value="checksum")
@patch("web3.Web3.HTTPProvider", return_value="provider")
def test_get_jwt(
    mocked_provider, mocked_checksum, mocked_random_msg, mock_sign_message, _
):
    with requests_mock.Mocker() as req_mocker:
        req_mocker.post(
            "https://graphql-gateway.axieinfinity.com/graphql",
            json={
                "data": {
                    "createAccessTokenWithSignature": {"accessToken": "test-token"}
                }
            },
        )
        c = Claim(account="ronin:foo", private_key="0xbar", acc_name="test_acc")
    ...
    mock_sign_message.assert_called_with(
        encode_defunct(text="random_msg"), private_key=c.private_key
    )
...
@patch("web3.Web3.toChecksumAddress", return_value="checksum")
@patch("web3.Web3.HTTPProvider", return_value="provider")
def test_jwq_fail_req_content(
    mocked_provider, mocked_checksum, mocked_random_msg, mock_sign_message, _
):
    with requests_mock.Mocker() as req_mocker:
        req_mocker.post(
            "https://graphql-gateway.axieinfinity.com/graphql", json={"data": {}}
        )
        c = Claim(account="ronin:foo", private_key="0xbar", acc_name="test_acc")
        jwt = c.get_jwt()
        expected_payload = {
...
@patch("web3.Web3.toChecksumAddress", return_value="checksum")
@patch("web3.Web3.HTTPProvider", return_value="provider")
def test_jwq_fail_req_content_2(
    mocked_provider, mocked_checksum, mocked_random_msg, mock_sign_message, _
):
    with requests_mock.Mocker() as req_mocker:
        req_mocker.post(
...
@patch("web3.Web3.toChecksumAddress", return_value="checksum")
@patch("web3.Web3.HTTPProvider", return_value="provider")
async def test_claim_execution(
    mocked_provider,
    mocked_checksum,
    mocked_contract,
    ...
    with patch.object(builtins, "open", mock_open(read_data='{"foo": "bar"}')):
        with requests_mock.Mocker() as req_mocker:
            ...
            c = Claim(
                account="ronin:foo",
                private_key="0x00003A01C01173D676B64123",
                acc_name="test_acc",
            )
            await c.execute()
    mocked_provider.assert_called_with(
    ...
    assert (
        "SLP Claimed! New balance for account test_acc (ronin:foo) is: 123"
        in caplog.text
    )
    with open(log_file) as f:
        ...
        assert len(lf) == 1
    assert (
        "Important: SLP Claimed! New balance for account test_acc (ronin:foo) is: 123"
        in lf[0]
    )
    await async_cleanup_log_file(log_file)
...
@patch("web3.Web3.toChecksumAddress", return_value="checksum")
@patch("web3.Web3.HTTPProvider", return_value="provider")
async def test_execution_failed_get_blockchain(
    mocked_provider,
    mocked_checksum,
    mocked_contract,
    moocked_check_balance,
    ...
    with patch.object(builtins, "open", mock_open(read_data='{"foo": "bar"}')):
        with requests_mock.Mocker() as req_mocker:
            req_mocker.post(
                ...
                json={
                    "blockchain_related": {
                        "signature": {"amount": "", "timestamp": 0, "signature": ""}
                    }
                },
            )
            c = Claim(
