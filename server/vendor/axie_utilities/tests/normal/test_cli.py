...
def test_payout_file_check_fail(caplog):
    with patch.object(sys, "argv", ["", "payout", "p_file.json", "s_file.json"]):
        cli.run_cli()
    assert (
        "Please provide a correct path to the file. Path provided: p_file.json"
        in caplog.text
    )
...
def test_generate_breedings_file_check_fail(caplog):
    with patch.object(
        sys, "argv", ["", "generate_breedings", "b_file.csv", "b_file.json"]
    ):
        cli.run_cli()
    assert (
        "Please provide a correct path to the file. Path provided: b_file.json"
        in caplog.text
    )
    assert "Please review your file paths and re-try." in caplog.text
...
def test_generate_transfers_file_check_fail(caplog):
    with patch.object(
        sys, "argv", ["", "generate_transfer_axies", "t_file.csv", "t_file.json"]
    ):
        cli.run_cli()
    assert (
        "Please provide a correct path to the file. Path provided: t_file.json"
        in caplog.text
    )
    assert "Please review your file paths and re-try." in caplog.text
...
def test_secret_gen_file_check_fail(caplog):
    with patch.object(
        sys, "argv", ["", "generate_secrets", "p_file.json", "s_file.json"]
    ):
        cli.run_cli()
    assert (
        "Please provide a correct path to the file. Path provided: s_file.json"
        in caplog.text
    )
    assert "Please review your file paths and re-try." in caplog.text
...
def test_generate_secrets_already_there(tmpdir):
    ...
    f2.write(
        '{"ronin:<account_s1_address>": "hello", "ronin:<account_s2_address>": "test"}'
    )
...
def test_generate_payments_file(tmpdir):
    ...
    f1.write(
        "Name,AccountAddress,ScholarPayoutAddress,ScholarPercent,ScholarPayout\n"
    ...
    f2.write("{}")
    with patch.object(
        builtins, "input", lambda _: "ronin:9fa1bc784c665e683597d3f29375e45786617550"
    ):
    ...
    assert json.loads(f2.read()) == [
        {
            ...
            "Transfers": [
                {"AxieId": 1231, "ReceiverAddress": "ronin:<whowillgetanaxie>"},
                {"AxieId": 1232, "ReceiverAddress": "ronin:<whowillgetanaxie>"},
                {"AxieId": 1233, "ReceiverAddress": "ronin:<whowillgetanaxie>"},
            ],
        },
        {
            "AccountAddress": "ronin:<whohasanaxie2>",
            "Transfers": [
                {"AxieId": 1234, "ReceiverAddress": "ronin:<whowillgetanaxie>"},
                {"AxieId": 1235, "ReceiverAddress": "ronin:<whowillgetanaxie>"},
                {"AxieId": 1236, "ReceiverAddress": "ronin:<whowillgetanaxie>"},
            ],
        },
    ]
...
def test_payout_takes_auto_parameter(
    mock_prepare_payout, mock_verify_input, mocked_paymentsmanager, tmpdir
):
    f1 = tmpdir.join("file1.json")
    f1.write(
        '{"Scholars":[{"Name": "Acc1", "AccountAddress": "ronin:<account_s1_address>"}]}'
...
@patch("axie.AxiePaymentsManager.verify_inputs")
@patch("axie.AxiePaymentsManager.prepare_payout")
def test_payout_takes_auto_parameter_yes(
    mock_prepare_payout, mock_verify_inputs, mocked_paymentsmanager, tmpdir
):
    f1 = tmpdir.join("file1.json")
    f1.write(
        '{"Scholars":[{"Name": "Acc1", "AccountAddress": "ronin:<account_s1_address>"}]}'
...
def test_transfer_file_check_fail(caplog):
    with patch.object(
        sys, "argv", ["", "transfer_axies", "t_file.json", "s_file.json"]
    ):
        cli.run_cli()
    assert (
        "Please provide a correct path to the file. Path provided: t_file.json"
...
def test_transfer(
    mock_verify_inputs, mock_prepare_transfers, mock_transfersmanager, tmpdir
):
    f1 = tmpdir.join("file1.json")
    f1.write('{"ronin:<account_s1_address>": "hello"}')
    f2 = tmpdir.join("file2.json")
...
def test_transfer_secure(
    ...
    f2.write('{"ronin:<account_s1_address>": "hello"}')
    with patch.object(
        sys, "argv", ["", "transfer_axies", str(f1), str(f2), "--safe-mode"]
    ):
        cli.run_cli()
    mock_verify_inputs.assert_called_with()
    mock_prepare_transfers.assert_called_with()
...
def test_axie_morphing(
    mock_veritfy_inputs,
    mock_morphing_execute,
    mock_find_axies,
    mock_axies_init,
    mock_morphingmanager,
    tmpdir,
):  # noqa
    ...
    mock_morphingmanager.assert_has_calls(
        [call([1, 2, 3], "foo", str(f)), call([1, 2, 3], "bar", str(f))]
    )
...
def test_breeding(
    mock_verify_inputs, mock_execute_breeding, mock_breedingmanager, tmpdir
):
