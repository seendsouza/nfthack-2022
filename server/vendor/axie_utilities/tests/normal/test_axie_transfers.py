...
def test_transfer_manager_init(mocked_load_json):
    ...
    mocked_load_json.assert_has_calls(calls=[call(transfers_file), call(secrets_file)])
...
def test_transfer_manager_prepare_transfers(
    mocked_execute_transfers, mocked_load_json, mock_axies
):
    transfers_file = "sample_transfers_file.json"
    secrets_file = "sample_secrets_file.json"
    atm = AxieTransferManager(transfers_file, secrets_file)
...
@patch("axie.transfers.load_json")
@patch("axie.transfers.AxieTransferManager.execute_transfers")
def test_transfer_manager_prepare_transfers_only_available(
    mocked_execute_transfers, mocked_load_json, mock_axies
):
    transfers_file = "sample_transfers_file.json"
    secrets_file = "sample_secrets_file.json"
...
@patch("axie.transfers.load_json")
@patch("axie.transfers.AxieTransferManager.execute_transfers")
def test_transfer_manager_prepare_transfers_secure(
    mocked_execute_transfers, mocked_load_json, _
):
    transfers_file = "sample_transfers_file.json"
    secrets_file = "sample_secrets_file.json"
    atm = AxieTransferManager(transfers_file, secrets_file, True)
...
def test_execute_calls_web3_functions(
    ...
    t = Transfer("ronin:from_ronin", "0xsecret", "ronin:to_ronin", 123)
    with patch.object(
    ...
    assert (
        "Axie Transfer of axie (123) from account (ronin:from_ronin) to account "
