...
def generate_secrets_file(payments_file_path, secrets_file_path=None):
    ...
    for acc in payments["Scholars"]:
        if acc["AccountAddress"] not in secrets:
            ...
            while new_secret == "":
                msg = (
                    f"Please provide private key for account {acc['Name']}. "
...
def run_cli():
    ...
    if args["payout"]:
        ...
        if check_file(payments_file_path) and check_file(secrets_file_path):
            ...
            apm = AxiePaymentsManager(
                payments_file_path, secrets_file_path, auto=args["--yes"]
            )
            apm.verify_inputs()
            apm.prepare_payout()
        else:
        ...
        if (
            secrets_file_path
            and check_file(secrets_file_path)
            and check_file(payments_file_path)
            or not secrets_file_path
            and check_file(payments_file_path)
        ):
            logging.info(
                "If you do not know how to get your private keys, check: "
        ...
        if check_file(transfers_file_path) and check_file(secrets_file_path):
            atm = AxieTransferManager(
                transfers_file_path, secrets_file_path, secure=secure
            )
            atm.verify_inputs()
        ...
        if check_file(breedings_file_path) and check_file(secrets_file_path):
            ...
            while payment_account == "":
                ...
                if len(msg) == 46 and msg.startswith("ronin:"):
                    ...
                    logging.info(f"Ronin provided ({msg}) looks wrong, try again.")
            abm = AxieBreedManager(
                breedings_file_path, secrets_file_path, payment_account
            )
            abm.verify_inputs()
            abm.execute()
        else:
