    ...
    def verify_inputs(self):
        ...
        for acc in self.transfers_file:
            if acc["AccountAddress"].lower() not in self.trezor_config:
                logging.critical(
                    f"Account '{acc['AccountAddress']}' is not present in trezor config file, "
        ...
        for acc in self.transfers_file:
            ...
            for axie in acc["Transfers"]:
                if not self.secure or (
                    self.secure
                    and axie["ReceiverAddress"].lower() in self.trezor_config
                ):
                    ...
                    if axie["AxieId"] in axies_in_acc:
                        t = TrezorTransfer(
                            ...
                            client=get_default_client(
                                ui=CustomUI(
                                    passphrase=self.trezor_config[
                                        acc["AccountAddress"].lower()
                                    ]["passphrase"]
                                )
                            ),
                            bip_path=self.trezor_config[acc["AccountAddress"].lower()][
                                "bip_path"
                            ],
