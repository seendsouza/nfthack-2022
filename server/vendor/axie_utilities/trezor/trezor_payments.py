...
class TrezorPayment:
    def __init__(
        self, name, payment_type, client, bip_path, from_acc, to_acc, amount, summary
    ):
        self.w3 = Web3(
            Web3.HTTPProvider(
                RONIN_PROVIDER_FREE,
    ...
    def execute(self):
        ...
        send_tx = self.contract.functions.transfer(
            ...
            {"chainId": 2020, "gas": self.gas, "gasPrice": self.gwei, "nonce": nonce}
        )
    ...
    def verify_inputs(self):
        ...
        for acc in self.payments_file["Scholars"]:
            if acc["AccountAddress"].lower() not in self.trezor_config:
                logging.critical(
                    f"Account '{acc['Name']}' is not present in trezor config file, please re-run setup."
                )
                validation_success = False
        if not validation_success:
            ...
            logging.critical(
                "If your problem is with trezor_config.json, delete it and re-run trezor setup command."
            )
            sys.exit()
    ...
    def prepare_payout(self):
        for acc in self.scholar_accounts:
            client = get_default_client(
                ui=CustomUI(
                    passphrase=self.trezor_config[acc["AccountAddress"].lower()][
                        "passphrase"
                    ]
                )
            )
            bip_path = parse_path(
                self.trezor_config[acc["AccountAddress"].lower()]["bip_path"]
            )
