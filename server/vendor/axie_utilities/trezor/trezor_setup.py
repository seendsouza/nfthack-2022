...
class TrezorAccountsSetup:
    ...
    def update_trezor_config(self):
        ...
        while non_configured_accs:
            ...
            while num_accs == 0:
                num_inp = input(
                    "Please input number of accounts in that passphrase (1-50): "
                )
                try:
            ...
            for i in range(num_accs):
                ...
                address = (
                    ethereum.get_address(client, parse_path(bip_path), True)
                    .lower()
                    .replace("0x", "ronin:")
                )
                if address in non_configured_accs:
                    self.trezor_config[address] = {
                        "passphrase": pf,
                        "bip_path": bip_path,
                    }
        ...
        file_path = (
            self.trezor_config_file if self.trezor_config_file else "trezor_config.json"
        )
