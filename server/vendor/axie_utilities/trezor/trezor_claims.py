...
class TrezorClaim(TrezorAxieGraphQL):
    ...
    def has_unclaimed_slp(self):
        ...
        try:
            ...
            logging.critical(
                f"Failed to check if there is unclaimed SLP for acc {self.acc_name} "
    ...
    async def execute(self):
        ...
        if not unclaimed:
            logging.info(
                f"Important: Account {self.acc_name} ({self.account.replace('0x', 'ronin:')}) "
                "has no claimable SLP"
            )
            return
        logging.info(
            f"Account {self.acc_name} ({self.account.replace('0x', 'ronin:')}) has "
            f"{unclaimed} unclaimed SLP"
        )
        jwt = self.get_jwt()
        if not jwt:
            logging.critical(
                "Important: Skipping claiming, we could not get the JWT for account "
                f"{self.account.replace('0x', 'ronin:')}"
            )
            return
        headers = {"User-Agent": self.user_agent, "authorization": f"Bearer {jwt}"}
        ...
        try:
            ...
            logging.critical(
                f"Error! Executing SLP claim API call for account {self.acc_name}"
        ...
        if 200 <= response.status_code <= 299:
            ...
            if not signature or not signature["signature"]:
                logging.critical(
                    f"Account {self.acc_name} ({self.account.replace('0x', 'ronin:')}) had no signature "
                    "in blockchain_related"
                )
                return
        else:
            logging.info(
                f"Important: Claim for account {self.acc_name} ({self.account.replace('0x', 'ronin:')}) "
                "had to be skipped"
            )
            return
        ...
        while True:
            try:
                ...
                logging.debug(
                    f"Waiting for claim for {self.acc_name} ({self.account.replace('0x', 'ronin:')}) to "
        ...
        if success:
            logging.info(
                f"Important: SLP Claimed! New balance for account {self.acc_name} "
...
class TrezorAxieClaimsManager:
    def __init__(self, payments_file, trezor_config):
        self.trezor_config, self.acc_names = self.load_trezor_config_and_acc_name(
            trezor_config, payments_file
        )
    ...
    def prepare_claims(self):
        claims_list = [
            TrezorClaim(
                ...
                client=get_default_client(
                    ui=CustomUI(passphrase=self.trezor_config[acc]["passphrase"])
                ),
        ...
        loop.run_until_complete(
            asyncio.gather(*[claim.execute() for claim in claims_list])
        )
