...
.utils import load_json, ImportantLogsFilter
...
class TrezorMorph(TrezorAxieGraphQL):
    ...
    def execute(self):
        ...
        try:
            ...
            logging.critical(
                f"Important! Axie {self.axie} in {self.account} is not ready to be morphed!"
            )
            return
        ...
        if 200 <= response.status_code <= 299:
            if response.json().get("data") and response.json()["data"].get("morphAxie"):
                logging.info(
                    f"Important: Axie {self.axie} in {self.account} correctly morphed!"
                )
            else:
                logging.info(
                    f"Important: Somethin went wrong morphing axie {self.axie} in {self.account}"
                )
        else:
            logging.critical(
                f"Important! Axie {self.axie} in {self.account} is not ready to be morphed!"
...
class TrezorAxieMorphingManager:
    ...
    def verify_inputs(self):
        if self.account not in self.trezor_config:
            logging.critical(
                f"Account '{self.account}' is not present in trezor config, please re-run trezor setup."
            )
            sys.exit()
        ...
        for axie in self.axie_list:
            m = TrezorMorph(
                ...
                client=get_default_client(
                    ui=CustomUI(
                        passphrase=self.trezor_config[self.account]["passphrase"]
                    )
                ),
